...
def moex_history(
    ...
    for i in tqdm(range(len(secids))):
        market_obj = {"engines": engine, "markets": market, "securities": secids[i]}
        ...
        for start_index in tqdm(range(0, total + 1, page_size)):
            ...
            response = requests.get(url + query, params=query_params, verify=False)
            asset_price = response.json()
            data = asset_price["history"]["data"]
            cols = list(map(lambda x: x.lower(), asset_price["history"]["columns"]))
