...
def position_rotator(price, freq, shift_days=0, mode=2):
    ...
    if mode == 2:
        df_maxs["downtrend"] = (df_maxs["max_f"] - df_maxs["max_b"]) / df_maxs[
            "max_b"
        ] > 0
        ...
        if len(df_mins) >= 4:
            ...
            for right, left in sup_points_idx:
                price_increment = (
                    df_mins.loc[right, "min"] - df_mins.loc[left, "min"]
                ) / (right - left).days
        ...
        if mean_price_incr > 0:
            ...
            sup_lvl = (
                delta_days * mean_price_incr + df_mins.loc[df_mins.index[-2], "min"]
            )
    ...
    min_max_dates = min_max.groupby("group").apply(
        lambda x: x["value"].idxmax()
        if x["action"].iloc[0] == "sell"
        else x["value"].idxmin(),
