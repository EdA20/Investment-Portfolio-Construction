...
def write_log_file(info_to_log: Union[Dict, str] = None):
    ...
    is_file_name = list(map(lambda x: x == file_name, os.listdir(PROJECT_ROOT / 'logs')))
    if any(is_file_name):
        filemode = 'a'
    else:
        filemode = 'w'
...
def read_logger(path: str = None):
    ...
    for path in paths:
        ...
        df_log['run_date'] = pd.to_datetime(path.split('_')[-1].split('.')[0], format='%Y%m%d')
...
def open_random_features_pnl_file(path=None):
    ...
    if random_features_pnl.shape[1] > 1:
        if random_features_pnl.shape[1] == 2:
            ...
            cols = ['pnl_mean', 'pnl_std', 'mean_median_perf_delta', 'mean_std_perf_delta']

        random_features_pnl = random_features_pnl\
            .set_axis(cols, axis=1)\
            .sort_values('pnl_mean', ascending=False)
...
class Dataset:
    ...
    def get_sample_weights(
        ...
        if mode == 'rotation_event':
            ...
            reaction_days = np.arange(1, 1 + weight_params['imptnt_obs_lag_reaction']).reshape(-1, 1)
            rotation_period_idx = (np.where(data[rotation_column].notna())[0] + reaction_days).ravel()
            ...
            data[weight_name] = data[weight_name].apply(
                lambda x: x * weight_params['return_mult'] if
                (x <= -weight_params['return_thresh']) or (x >= weight_params['return_thresh']) else
                0.01
            )
        ...
        if weight_smoothing.get('win_type') == 'exp':
            ...
            data[weight_name] = data[weight_name]\
                .rolling(window=weight_smoothing['window'], center=True, win_type='gaussian')\
    ...
    def get_batches(self, data, features=None):
        ...
        if features:
            ...
            if len(features_idx) != len(features):
                absent_features = list(set(features) - set(data.iloc[:, features_idx].columns))
                raise Exception(f'в data нет фичей {absent_features}')

        if (
            (self.splitter_kwargs['initial_window'] and self.splitter_kwargs['window_length']) or
            (not self.splitter_kwargs['initial_window'] and not self.splitter_kwargs['window_length'])
        ):
            raise Exception('initial_window исп-ся для расширящегося окна, window_length - для скользящего. Нужно выбрать что-то одно')
        ...
        oob_test_dates = pd.DatetimeIndex(set(data.index[-self.splitter_kwargs['step_length'] + 1:]) - set(dates[-1][1]))
        ...
        if self.splitter_kwargs['eval_obs'] > 1:
            ...
            dates = [
                (
                    train_dates[:-int(len(train_dates) * self.splitter_kwargs['eval_obs'])],
                    train_dates[-int(len(train_dates) * self.splitter_kwargs['eval_obs']):],
                    test_dates
                )
                for train_dates, test_dates in dates
        ...
        for splitted_dates in tqdm(dates, desc='position_rotator', disable=TQDM_DISABLE):
            ...
            if self.splitter_kwargs['eval_obs'] > 0:
                train, val, test = splitted_dates[0], splitted_dates[1], splitted_dates[2]
            ...
            for subset, (splitted_dts, united_dts) in train_val_test_dates.items():
                ...
                rotator = position_rotator(data.loc[united_dts, 'price'], **self.position_rotator_kwargs)
                ...
                if self.sample_weight_kwargs:
                    obs_weights = self.get_sample_weights(
                        data.loc[united_dts], f'{subset}_target', f'{subset}_weight', **self.sample_weight_kwargs
                    )
                    data[f'{subset}_weight'] = obs_weights
                else:
                    data[f'{subset}_weight'] = 1
                data[f'{subset}_target'] = (data[f'{subset}_target'].ffill() == 'buy').astype(int)
...
class StrategyModeller(Dataset):
    ...
    def get_model_output(self, batches, **kwargs):
        ...
        if model_name == 'catboost':
            train_infos = []
            if len(batches[0]) == 2:
                for pools, subset_dates in tqdm(batches, desc='batches', disable=TQDM_DISABLE):
                    pools = pools.copy()
                    ...
                    columns = ['from_date', 'till_date', 'mean_train_target', 'mean_test_target'] + list(features)
                    train_info = [
                        first_test_date, last_test_date,
                        mean_train_target, mean_test_target,
                ...
                for pools, subset_dates in tqdm(batches, desc='batches', disable=TQDM_DISABLE):
                    ...
                    model = CatBoostClassifier(
                        loss_function=FocalLossObjective(), eval_metric=FocalLossMetric(), **model_kwargs
                    )
                    model.fit(pools['train'], use_best_model=True, eval_set=pools['val'], verbose=False)
                    preds = model.predict_proba(pools['test'])[:, 1]
            ...
            raise Exception(f'Модель {model_name} не засетаплена, исп-те название из перечня: {self.ALL_MODELS}')
    ...
    def get_predictions(self, batches):
        ...
        all_preds = pd.concat(all_preds, axis=1).set_axis([f'preds_seed_{i}' for i in rng], axis=1)
    ...
    def base_strategy_peformance(self, strat_data, preds, plot=False):
        ...
        strat_data['turnover'] = abs(strat_data['preds'] - strat_data['preds'].shift()).cumsum() / (((strat_data.index - strat_data.index[0]).days + 1)/365)
        strat_data['preds'] = strat_data['preds'].shift()
        strat_data['is_bench_long'] = strat_data['preds'] >= 0.5

        cols = ['price', 'ruonia', 'ruonia_daily', 'preds', 'is_bench_long', 'price_return', 'turnover']
        ...
        if self.prob_to_weight:
            ...
            res['bench_long_weight'] = res['preds'].apply(lambda x: 1 if x >= 0.5 else 0)
        ...
        test_subset_pnl = res.groupby(res['test_subset_idx'])[['price_return', 'strat_return']].apply(lambda x: (1 + x).prod() - 1)
        test_subset_outperf = test_subset_pnl['strat_return'] - test_subset_pnl['price_return']

        n_test_subsets = np.array([i for i in range(1, len(test_subset_outperf) + 1)])
        weight_function_1 = lambda x, power: x ** power / sum(x ** power)
        weight_function_2 = lambda x, base: 1.0 / (1.0 + base ** (-x + 36.0)) / sum(1.0 / (1.0 + base ** (-x + 36.0)))
        weights = (
            weight_function_2(n_test_subsets, 1.1) + weight_function_1(n_test_subsets, 1/2) /
            sum(weight_function_2(n_test_subsets, 1.1) + weight_function_1(n_test_subsets, 1/2))
        )
        mean_test_subset_outperf = 100 * test_subset_outperf.mean()
        weighted_test_subset_outperf = 100 * sum(test_subset_outperf * weights)
        weighted_test_sharpe = weighted_test_subset_outperf / (res['strategy_perf'].std() / 100)

        print('ML Strategy Outperformance:', res['strategy_perf'].iloc[-1] - res['bench_perf'].iloc[-1])
        ...
        print('Weighted Outperformance Sharpe Metric:', weighted_test_sharpe, end='\n\n')
        ...
        if self.log_collector:
            ...
            self.log_collector['weighted_test_subset_outperf'] = weighted_test_subset_outperf
...
if __name__ == '__main__':
    ...
    group_median_perf_delta = df.groupby(['run_date', 'features'])['median_perf_delta'].mean().reset_index()
    df = pd.merge(df, group_median_perf_delta, how='left', on=['run_date', 'features'], suffixes=('', '_mean'))
    max_median_perf_delta = df.loc[df.groupby('run_date')['median_perf_delta_mean'].idxmax()].sort_values('run_date', ascending=False)
