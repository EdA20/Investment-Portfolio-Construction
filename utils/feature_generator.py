from utils import (
    EXOG_DATA_PRICE_FILES, EXOG_DATA_RATE_FILES, EXOG_DATA_OTHER_FILES, ALL_EXOG_DATA_FILES,
    ENDOG_DATA_FOLDER, EXOG_DATA_FOLDER, ALL_DATA_FILES, ALL_DATA_PATH
)
...
class TechnicalIndicators():
    ...
    def rsi(self, col='close', windows=[10], lags=[], moving_average_type='simple'):
        df = pd\
            .concat([self.data[f'{col}'], self.data[f'{col}'].shift()], axis=1)\
            .set_axis([f'{col}_curr', f'{col}_prev'], axis=1)
        df['U'] = df.apply(
            lambda x: x[f'{col}_curr'] - x[f'{col}_prev'] if x[f'{col}_curr'] > x[f'{col}_prev'] else 0,
            axis=1
        )
        df['D'] = df.apply(
            lambda x: x[f'{col}_prev'] - x[f'{col}_curr'] if x[f'{col}_curr'] < x[f'{col}_prev'] else 0,
            axis=1
        )
        data = []
        for window in windows:
            if moving_average_type == 'simple':
                ...
                df[f'rsi_ew_{window}'] = (
                    100 *
                    df['U'].ewm(alpha=1/window, min_periods=window, adjust=False).mean() /
                    (df['U'].ewm(alpha=1/window, min_periods=window, adjust=False).mean() + df['D'].ewm(alpha=1/window, min_periods=window, adjust=False).mean())
                )
            else:
                raise Exception(f'moving average type can be only simple or exponential, "{moving_average_type}" was passed')
    ...
    def macd(self, col='close', short_windows=[12], long_windows=[26], signal_windows=[9], cols_to_use=[0,1,2]):
        windows = zip(short_windows, long_windows, signal_windows)
        data = []
        for (short_window, long_window, signal_window) in windows:
            macd_line = (
                self.data[col].ewm(alpha=1/short_window, min_periods=short_window, adjust=False).mean() -
                self.data[col].ewm(alpha=1/long_window, min_periods=long_window, adjust=False).mean()
            )
            signal_line = macd_line.ewm(alpha=1/signal_window, min_periods=signal_window, adjust=False).mean()
            ...
            macd_info = pd.concat([macd_line, signal_line, macd_delta], axis=1).set_axis(cols, axis=1)
    ...
    def stochastic_oscillator(self, col='close', high_col='high', low_col='low', windows_k=[5], windows_n=[3], cols_to_use=[0,1], moving_average_type='simple'):
        windows = zip(windows_k, windows_n)
        data = []
        for (window_k, window_n) in windows:
            oscillator = (
                (self.data[col] - self.data[low_col].rolling(window_k).min()) /
                (self.data[high_col].rolling(window_k).max() - self.data[low_col].rolling(window_k).min())
            )
            oscillator -= 0.5
            if moving_average_type == 'simple':
                weightning_name = 'sw'
                ...
                weightning_name = 'ew'
                smoothed_oscillator = oscillator.ewm(alpha=1/window_n, min_periods=window_n, adjust=False).mean()
            else:
                raise Exception(f'moving average type can be only simple or exponential, "{moving_average_type}" was passed')

            cols = [f'oscillator_{weightning_name}_{window_k}', f'smoothed_oscillator_{weightning_name}_{window_n}']
            oscillators = pd.concat([oscillator, smoothed_oscillator], axis=1).set_axis(cols, axis=1)
    ...
    def mfi(self, windows=[10]):
        data = []
        for window in windows:
            typical_price = (self.data['close'] + self.data['low'] + self.data['high']) / 3
            positive, negative = typical_price > typical_price.shift(), typical_price < typical_price.shift()
            ...
            money_flow_index = (
                ...
                (money_flow_positive.rolling(window).sum() + money_flow_negative.rolling(window).sum())
            )
    ...
    def atr(self, windows=[10], moving_average_type='simple'):
        data = []
        for window in windows:
            df = pd.concat([self.data['close'].shift(), self.data['low'], self.data['high']], axis=1).set_axis(['close_prev', 'low', 'high'], axis=1)
            true_range = df.apply(lambda x: max(
                x['high'] - x['low'], abs(x['high'] - x['close_prev']), abs(x['low'] - x['close_prev'])
            ), axis=1)
            ...
            if moving_average_type == 'simple':
                ...
                raise Exception(f'moving_average_type takes only args "simple" or "exponential", "{moving_average_type}" was passed')
    ...
    def disparity(self, short_windows=[12], long_windows=[26], moving_average_type='simple'):
        windows = zip(short_windows, long_windows)
        data = []
        for (short_window, long_window) in windows:
            if moving_average_type == 'simple':
                disparity = (self.data['close'].rolling(short_window).mean() / self.data['close'].rolling(long_window).mean())
            elif moving_average_type == 'exponential':
                disparity = (
                    self.data['close'].ewm(alpha=1/short_window, min_periods=short_window, adjust=False).mean() /
                    self.data['close'].ewm(alpha=1/long_window, min_periods=long_window, adjust=False).mean()
                )
            else:
                raise Exception(f'moving_average_type takes only args "simple" or "exponential", "{moving_average_type}" was passed')
    ...
    def custom_aroon(self, col='close', windows=[14], cols_to_use=[0,1,2]):
        data = []
        for window in windows:
            aroon_up = self.data[col].rolling(window).apply(lambda x: 1 - x.argmax() / window)
            aroon_down = self.data[col].rolling(window).apply(lambda x: 1 - x.argmin() / window)
            aroon_delta = aroon_down - aroon_up

            cols = [f'aroon_up_{window}', f'aroon_down_{window}', f'aroon_delta_{window}']
            aroon = pd.concat([aroon_up, aroon_down, aroon_delta], axis=1).set_axis(cols, axis=1)
    ...
    def bollinger_bands(self, col='close', n_sigmas=3, windows=[6, 12, 18], cols_to_use=[0,1,2,3]):
        data = []
        for window in windows:
            ...
            cols = [f'lower_band_{window}', f'upper_band_{window}', f'bb_buy_ind_{window}', f'bb_sell_ind_{window}']
            bollinger_bands = pd.concat([lower_band, upper_band, buy_ind, sell_ind], axis=1).set_axis(cols, axis=1)
    ...
    def distribution_oscillator(self, cols=['return'], windows=[252, 252*1.5], roc_windows=[61, 121]):
        data = []
        for roc_window in roc_windows:
            roc = self.data[cols].rolling(roc_window).apply(lambda x: (1 + x).prod() - 1)
            for col in cols:
                for window in windows:
                    tqdm.pandas()
                    dist_osc = roc[col].rolling(window).progress_apply(lambda x: self._get_cdf(x, col))
    ...
    def vwap(self, price_col='close', volume_col='volume', windows=[61, 121]):
        ...
        for window in windows:
            ...
            vwap = price_vol.rolling(window).sum() / self.data[volume_col].rolling(window).sum()
...
def rolling_statistic_generator(
    ...
    stats = []
    del_cols = list(filter(lambda x: ('price' in x) and ('return' not in x), columns)) + EXOG_DATA_PRICE_FILES
    drop_cols = []
        ...
        for attr in attrs:
            for smoothing_type in smoothing_types:
                if (attr in simple_attrs) or (smoothing_type == 'simple'):
                    cols = [
                        i + f'_sw_{attr}_{w}' for i in columns
                    ]
                    drop_cols.extend(
                        [i + f'_sw_mean_{w}' for i in del_cols]
                    )
                    func = getattr(data.loc[:, columns].rolling(w), attr)
                else:
                    cols = [
                        i + f'_ew_{attr}_{w}' for i in columns
                    ]
                    drop_cols.extend(
                        [i + f'_ew_mean_{w}' for i in del_cols]
                    )
                    func = getattr(data.loc[:, columns].ewm(span=w, min_periods=w), attr)
        ...
        for attr in attrs:
            for smoothing_type in smoothing_types:
                if (attr in simple_attrs) or (smoothing_type == 'simple'):
                    ...
                    new_cols_div = [i + f'_sw_{attr}_{short_w}/{long_w}' for i in columns]
                    new_cols_subs = [i + f'_sw_{attr}_{short_w}_{long_w}' for i in columns]
                else:
                    short_w_cols = [i + f'_ew_{attr}_{short_w}' for i in columns]
                    long_w_cols = [i + f'_ew_{attr}_{long_w}' for i in columns]
                    new_cols_div = [i + f'_ew_{attr}_{short_w}/{long_w}' for i in columns]
                    new_cols_subs = [i + f'_ew_{attr}_{short_w}_{long_w}' for i in columns]
...
def time_features_generator(data):
    ...
    time_features = pd.DataFrame(time_features.toarray(), index=data.index, columns=cols)
...
def feature_construction(data, ta_methods, stat_gen_kwargs):
    ...
    ta_data = [getattr(ti, method)(**ta_method_kwargs) for method, ta_method_kwargs in ta_methods.items()]
...
def data_generator(path: str = None, exog_data_file_names: List[str] = None, tsfresh_features: bool = False):

    if exog_data_file_names is None:
        exog_data_file_names = ALL_EXOG_DATA_FILES
    exog_data_file_paths = [EXOG_DATA_FOLDER / (name + '.xlsx') for name in exog_data_file_names]
    ...
    rate_files_in = list(filter(lambda x: x in exog_data_cols.keys(), EXOG_DATA_RATE_FILES))
    ...
    df = pd.merge(
        instrument, exog_data, 'left', left_index=True, right_index=True
    )
    df = df.interpolate(method='linear', limit_direction='forward', limit_area='inside', axis=0)
    df['price_return'] = df['price'].pct_change()

    # код написан с расчетом на, что в exog_data_file_names захочется передавать не все файлы
    # подсчет процентного изменения ценовых (и не только) временных рядов
    calc_return_files_in = list(
        filter(lambda x: x in exog_data_cols.keys(), EXOG_DATA_PRICE_FILES + EXOG_DATA_OTHER_FILES)
    )
    ...
    df['price_vol_adj'] = df['price'].iloc[0] * (1 + df['price_vol_adj_return']).cumprod()
    ...
    ta_methods = {
        'rsi': dict(col='price', windows=[10, 21, 63, 126], moving_average_type='simple'),
        'stochastic_oscillator': dict(
            col='price', high_col='price', low_col='price',
            windows_k=[10, 21, 63, 126], windows_n=[5, 10, 21, 21],
            moving_average_type='simple', cols_to_use=[0]
        ),
        'stat_to_price_ratios': dict(col='price', stats=['mean', 'min', 'median', 'max'], windows=[10, 21, 63, 126, 252]),
        'roc': dict(cols=return_cols, windows=[10, 21, 63, 126, 252]),
        'custom_aroon': dict(col='price', windows=[126, 252, 504]),
        'distribution_oscillator': dict(cols=['price_return'], windows=[252, 378], roc_windows=[21, 63, 126, 252]),
    }
    ...
    not_stationary_cols = list(filter(lambda x: x in exog_data_cols.keys(), EXOG_DATA_PRICE_FILES)) + ['price_vol_adj']
...
def position_rotator(price, freq, shift_days=0, mode=2):
    ...
    if mode == 2:
        ...
        df_maxs.loc[df_maxs.index > last_max_adj_idx, 'global_max'] = False
    ...
    min_max = pd.concat(res).sort_index()
    min_max['group'] = (min_max['action'].shift() != min_max['action']).cumsum()
    min_max_dates = min_max.groupby('group').apply(
        lambda x: x['value'].idxmax() if x['action'].iloc[0] == 'sell' else x['value'].idxmin(),
        include_groups=False
    )
    min_max = min_max.loc[min_max_dates]

    if not price.index[0] == min_max.index[0]:
        min_max.loc[price.index[0]] = [price[price.index[0]], min_max.iloc[1, 1], 0]
        min_max = min_max.sort_index()
