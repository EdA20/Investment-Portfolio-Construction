...
class SampleStrategy(StrategyModeller):
    ...
    def sample_random_seed_strategy(self, size=100):
        ...
        for i in tqdm(range(size)):
            df_random_seed_pnls = pd.DataFrame.from_dict(random_seed_pnls, orient='index')\
                .reset_index()\
    ...
    def sample_shifted_strategy(self, choose_before='2014-01-01', init_window_resize=0, size=100, plot_with_date_normalization=True):
        ...
        for from_date, new_init_w in tqdm(dates_windows):
            df_pnl = pd.DataFrame.from_dict(pnl, orient='index')\
                .reset_index()\
        ...
        if plot_with_date_normalization:
            begin_date = sample_strategy_returns.loc[:, sample_strategy_returns.columns.max()].notna().idxmax()
    ...
    def random_features_sampling(
        ...
        ns_features_to_select = np.random.choice(range(min_amount_features, max_amount_features + 1), size=n_trials)
        random_features_pnl = {}
        counter = dict(zip(all_features, np.zeros(len(all_features))))

        for n_features_to_select in tqdm(ns_features_to_select):
            features = list(np.random.choice(all_features, replace=False, size=n_features_to_select))
    ...
    def get_good_features(counter, max_amount_features, fraction=0.1):
        prob_features_to_choose = {key: value / sum(counter.values()) for key, value in counter.items()}
    ...
    def random_features_metric_sampling(
        ...
        exp_feature_metric_accum = {key: np.exp(value) for key, value in feature_metric_accum.items()}
        feature_softmax = {key: value / sum(exp_feature_metric_accum.values()) for key, value in exp_feature_metric_accum.items()}

        for n_features_to_select in tqdm(ns_features_to_select, desc='n_trials'):

            features = np.random.choice(
                list(feature_softmax.keys()), replace=False, size=n_features_to_select, p=list(feature_softmax.values())
            )
            ...
            for i in range(1, n_seed + 1):
                ...
                metric = res['weighted_perf_delta'].mean() / (res['strategy_perf'].std() / 100) * 10
                for feature in features:
    ...
    def random_features_group_sampling(
        ...
        for group, amounts in per_feature_group_amount.items():
            group_random_amounts = np.random.choice(
                range(amounts['min_amount_features'], amounts['max_amount_features'] + 1),
                size=n_trials
            )
        ...
        for n_selected_features_in_group in tqdm(ns_selected_features_in_group, desc='n_trials'):
            ...
            for i, (exog_feature, generated_features) in enumerate(feature_groups.items()):
                selected_generated_features = list(
                    np.random.choice(generated_features, size=n_selected_features_in_group[i], replace=False)
                )
            ...
            for i in range(1, n_seed + 1):
                ...
                output = self.startegy_performance(features=features, seed=i)
                res = output['res'].copy()
                if res['strategy_perf'].iloc[-1] < pnl_thresh or res['perf_delta'].median() < perf_delta_median_thresh:
                    break
            ...
            if n_passed_hard_counter % n_passed_hard_counter_thresh == 0:
                good_features = self.get_good_features(hard_counter, max_amount_features, fraction=1)
                good_feature_trials = 0
    ...
    def optuna_optimization_sampling(
        ...
        if metric not in _all_metrics:
            raise Exception(f'No such metric - "{metric}", available metrics: {_all_metrics}')
        ...
        study.optimize(
            lambda trial: self._objective(trial, metric, all_features, n_seed, json_path),
            n_trials=n_trials,
        )
    ...
    def _objective(self, trial, metric, all_features, n_seed, json_path):
        ...
        is_feature_used = [trial.suggest_categorical(name, [True, False]) for name in all_features]
