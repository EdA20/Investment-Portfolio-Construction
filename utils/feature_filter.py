...
def group_features(features_to_group: List[str], grouping_lvl: str = 'base_cols'):
    ...
    for name, path in exog_data_name_path_dct.items():
        ...
        if grouping_lvl == 'base_cols':
            ...
            features_to_group = list(set(features_to_group) - set(feature_groups[group_name]))

        elif grouping_lvl == 'base_cols_attrs':
            for attr in attrs:
                for col in temp.columns:
                    features = list(filter(lambda x: x.startswith(col) and attr in x, features_to_group))
                    grouped_cols.extend(features)
                group_name = f'{name}_{attr}'
                feature_groups[group_name] = grouped_cols.copy()
                features_to_group = list(set(features_to_group) - set(feature_groups[group_name]))
        else:
            raise Exception('Incorrect grouping_lvl argument. Possible values = "base_cols", "base_cols_attrs"')
...
def corr_feature_filter(data, features: Union[Iterable, None] = None, corr_threshold: float = None):
    ...
    features_to_drop = [
        column for column in upper_matrix.columns if any(upper_matrix[column] > corr_threshold)
    ]
...
def var_feature_filter(data: pd.DataFrame, features: Union[Iterable, None] = None, var_threshold: float = None):
...
def feature_filter(data: pd.DataFrame, features: Union[Iterable, None] = None, **kwargs):
...
def cv_mutual_info(data: pd.DataFrame, features: Union[Iterable, None] = None, cv: int = 5):
