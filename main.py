...
from utils.feature_generator import data_generator, replace_old_feature_names, position_rotator
from utils.feature_filter import feature_filter, cv_mutual_info
# from utils.sampler import SampleStrategy
from utils.model import read_logger, Dataset, StrategyModeller
...
def main(parser):
    ...
    if args.features == 'all':
        ...
        group_median_perf_delta = trials.groupby(['run_date', 'features'])['median_perf_delta'].mean().reset_index()
        trials = pd.merge(
            trials, group_median_perf_delta, how='left', on=['run_date', 'features'], suffixes=('', '_mean')
        )
        max_median_perf_delta = trials.loc[trials.groupby('run_date')['median_perf_delta_mean'].idxmax()].sort_values('run_date', ascending=False)
        # features = max_median_perf_delta['features'].iloc[0]
        features = max_median_perf_delta.loc[max_median_perf_delta['run_date'] == '2025-03-25', 'features'].item()
        ...
        if 'feature_importance.xlsx' not in os.listdir('data'):
            ...
            feature_importance = pd.read_excel('data/feature_importance.xlsx', parse_dates=True)
            top_feature_importance = feature_importance.mean().iloc[4:-1].sort_values(ascending=False)
            features = list(
                top_feature_importance.loc[top_feature_importance > top_feature_importance.mean()].index[:100]
            )
